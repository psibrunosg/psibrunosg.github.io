...
function classificar(score: number): { nivel: string; cor: string; orientacao: string } {
  ...
  if (score <= 14) return { nivel: "Moderada", cor: "#B05D3A", orientacao: "Nivel moderado de ansiedade. É importante discutir esses resultados com seu psicologo." };
    ...
    <div className="min-h-screen bg-[var(--c-bg)] flex flex-col" data-theme="c">
      <header className="fixed top-0 left-0 right-0 z-50 px-6 py-4 bg-[var(--c-bg)]/95 backdrop-blur border-b border-[var(--c-border)]">
        <div className="max-w-2xl mx-auto flex items-center justify-between">
          ...
          {etapa === "form" && (
            <span className="text-xs text-[var(--c-muted)]">{atual + 1} / {perguntas.length}</span>
          )}
          {etapa !== "form" && <span />}
        </div>
      </header>
          ...
          <AnimatePresence mode="wait">
            ...
            {etapa === "intro" && (
              <motion.div key="intro" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="text-center">
                ...
                <p className="text-xs text-[var(--c-muted)] mb-10 italic">
                  Ferramenta de rastreio, nao de diagnostico. Resultados compartilhados com seu psicologo.
                </p>
                <button
                  onClick={() => setEtapa("form")}
                  className="inline-flex items-center gap-2 px-8 py-3 rounded-full text-white font-medium hover:opacity-90 transition-opacity"
                ...
                <div className="space-y-3">
                  {opcoes.map((op) => (
                    <button
                      ...
                      style={{
                        borderColor: respostas[atual] === op.valor ? "#4A6B47" : "var(--c-border)",
                        background: respostas[atual] === op.valor ? "#4A6B4718" : "var(--c-surface)",
                        color: "var(--c-text)",
                      }}
                    >
                      {op.label}
            ...
            {etapa === "dados" && (
              <motion.div key="dados" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
                ...
                <p className="text-[var(--c-muted)] mb-8 leading-relaxed">
                  Para enviar as respostas ao seu psicologo, informe seus dados abaixo.
                </p>
                <div className="space-y-4 mb-8">
                  <div>
                    <label className="text-sm font-medium text-[var(--c-text)] block mb-1.5">
                      Nome <span className="text-[var(--c-accent)]">*</span>
                    </label>
                    <input
                      type="text"
                      value={nome}
                  ...
                  <div>
                    <label className="text-sm font-medium text-[var(--c-text)] block mb-1.5">
                      Telefone / WhatsApp <span className="text-[var(--c-muted)] text-xs font-normal">(opcional)</span>
                    </label>
                    <input
                      type="tel"
                      value={telefone}
                ...
                <button
                  ...
                  className="w-full flex items-center justify-center gap-2 px-8 py-4 rounded-full text-white font-medium hover:opacity-90 disabled:opacity-50 transition-opacity"
                  style={{ background: "#4A6B47" }}
                >
                  {enviando ? "Enviando..." : <><Send size={16} /> Enviar respostas</>}
                </button>
                <p className="text-xs text-[var(--c-muted)] mt-4 text-center">
                  Seus dados sao protegidos pelo sigilo profissional.
                </p>
              </motion.div>
            )}
            ...
            {etapa === "resultado" && (
              <motion.div key="resultado" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center">
                <div className="w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-6 text-3xl font-bold text-white" style={{ background: resultado.cor }}>
                  {pontuacao}
                </div>
                <span className="text-xs tracking-widest uppercase font-semibold block mb-1" style={{ color: resultado.cor }}>
                  {resultado.nivel}
                </span>
                <h2 className="text-2xl font-semibold text-[var(--c-text)] mb-4" style={{ fontFamily: "var(--font-heading)" }}>
                  Respostas enviadas
                </h2>
                <p className="text-[var(--c-muted)] leading-relaxed mb-10 max-w-sm mx-auto">
                  {resultado.orientacao}
                </p>
                <p className="text-xs text-[var(--c-muted)] mb-8 italic">
                  GAD-7: escala validada de rastreio. Nao constitui diagnostico. Resultado interpretado pelo psicologo.
                </p>
                <div className="flex flex-col sm:flex-row gap-3 justify-center">
                  <Link to="/paciente" className="px-6 py-3 rounded-full border border-[var(--c-border)] text-[var(--c-text)] hover:border-[var(--c-accent)] transition-colors text-sm">
                    Outros questionarios
                  </Link>
                  <Link to="/" className="px-6 py-3 rounded-full bg-[var(--c-accent)] text-white text-sm hover:opacity-90 transition-opacity">
                    Voltar ao site
                  </Link>
                </div>
              </motion.div>
