...
const perguntas = [
  ...
  "Se sentir mal consigo mesmo(a), achar que é um fracasso ou que decepcionou sua família",
...
function classificar(score: number): { nivel: string; cor: string; orientacao: string } {
  ...
  if (score <= 14) return { nivel: "Moderada", cor: "#B05D3A", orientacao: "Nivel moderado. É importante discutir esses resultados com seu psicologo." };
            ...
            {etapa === "intro" && (
              <motion.div key="intro" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="text-center">
                ...
                <button
                  ...
                  className="inline-flex items-center gap-2 px-8 py-3 rounded-full bg-[var(--c-accent)] text-white font-medium hover:opacity-90 transition-opacity"
                >
              ...
              <motion.div key={"q" + atual} initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }} transition={{ duration: 0.25 }}>
                <div className="mb-2">
                  <div className="h-1.5 bg-[var(--c-border)] rounded-full overflow-hidden mb-6">
                    <div
                      ...
                      style={{ width: ((atual + 1) / perguntas.length) * 100 + "%", background: "var(--c-accent)" }}
                    />
                  </div>
                </div>
                ...
                {atual > 0 && (
                  <button
                    ...
                    className="mt-6 inline-flex items-center gap-1 text-sm text-[var(--c-muted)] hover:text-[var(--c-accent)] transition-colors"
                  >
            ...
            {etapa === "resultado" && (
              <motion.div key="resultado" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center">
                ...
                <p className="text-xs text-[var(--c-muted)] mb-8 italic">
                  PHQ-9: escala validada de rastreio. Nao constitui diagnostico. Resultado interpretado pelo psicologo.
                </p>
